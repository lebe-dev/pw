...
impl ClientIpExtractor {
    pub async fn middleware(
        ...
        debug!("Extracted client IP: {} (connection IP: {})", client_ip, addr.ip());
    ...
    fn extract_client_ip(headers: &HeaderMap, connection_ip: IpAddr) -> IpAddr {
        ...
        debug!("No valid proxy headers found, using connection IP: {}", connection_ip);
        connection_ip
    }
...
mod tests {
    ...
    fn test_extract_client_ip_from_x_forwarded_for() {
        ...
        headers.insert("x-forwarded-for", "203.0.113.195, 198.51.100.178".parse().unwrap());
        let connection_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 100));
    ...
    fn test_ipv6_addresses() {
        ...
        let result = ClientIpExtractor::extract_client_ip(&headers, connection_ip);
        assert_eq!(result, IpAddr::V6(Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0, 0, 0, 0x1)));
    }

    #[test]
    fn test_parse_forwarded_for_multiple_ips() {
        let result = ClientIpExtractor::parse_forwarded_for("203.0.113.195, 198.51.100.178, 192.168.1.1");
    ...
    fn test_is_valid_ip_format() {
        ...
        assert!(!ClientIpExtractor::is_valid_ip_format("a".repeat(46).as_str()));
    }

    #[test]
    fn test_security_header_validation() {
        let mut headers = HeaderMap::new();
        headers.insert("x-forwarded-for", "0.0.0.0<script>alert('xss')</script>".parse().unwrap());
        let connection_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 100));
    ...
    fn test_forwarded_for_with_ports() {
        let mut headers = HeaderMap::new();
        headers.insert("x-forwarded-for", "203.0.113.195:8080, 198.51.100.178:9000".parse().unwrap());
        let connection_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 100));
    ...
    fn test_ipv6_in_headers() {
        let mut headers = HeaderMap::new();
        headers.insert("x-forwarded-for", "2001:db8::8a2e:370:0:7334".parse().unwrap());
        let connection_ip = IpAddr::V6(Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0, 0, 0, 0x1));
        
        let result = ClientIpExtractor::extract_client_ip(&headers, connection_ip);
        assert_eq!(result, IpAddr::V6(Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0x8a2e, 0x370, 0, 0x7334)));
    }

    #[test]
    fn test_mixed_ipv4_ipv6_in_forwarded_for() {
        let mut headers = HeaderMap::new();
        headers.insert("x-forwarded-for", "203.0.113.195, 2001:db8::1, 198.51.100.178".parse().unwrap());
        let connection_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 100));
    ...
    fn test_private_ip_addresses_in_headers() {
        ...
        let test_cases = vec![
            ...
            ("fc00::1", IpAddr::V6(Ipv6Addr::new(0xfc00, 0, 0, 0, 0, 0, 0, 1))),
        ];
    ...
    fn test_is_valid_ip_format_comprehensive() {
        ...
        for ip in valid_ipv4 {
            assert!(ClientIpExtractor::is_valid_ip_format(ip), "Should be valid: {}", ip);
        }
        ...
        for ip in &valid_ipv6[..4] { // Skip the zone identifier one
            assert!(ClientIpExtractor::is_valid_ip_format(ip), "Should be valid: {}", ip);
        }
        ...
        for ip in invalid {
            assert!(!ClientIpExtractor::is_valid_ip_format(ip), "Should be invalid: {}", ip);
        }
    }
