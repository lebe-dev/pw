...
{#if !loading}
	{#if !notFound}
		...
		{:else}
			<div class="columns-2">
				<div class="column-xs ps-3">
					<button id="copy-to-clipboard" data-clipboard-target="#secret-url"
							class="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded cursor-pointer text-md">
						...
						{data.config.locale.secretUrlPage.copyButton}
					</button>
				</div>
				<div class="column-xs text-right pe-3">
					<button class="px-3 py-1.5 ms-3 bg-gray-300 rounded hover:bg-red-700 hover:text-white cursor-pointer text-md"
